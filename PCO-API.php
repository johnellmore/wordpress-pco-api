<?php 
	...
	class PlanningCenterOnline{
		 ...
		 * @return object
		 */
		public function getPlansByServiceId($serviceId, $includePast = false){
			$url = $this->paths['baseUrl'] . sprintf($this->paths['plans']['serviceTypes'],$serviceId);
			if ($includePast) $url .= '?all=true';
			return $this->fetch($url);
		}
		...
		protected function getAccessTokenFileName(){
			return dirname(__FILE__)."/.planningcenteronline_api.accesstoken";
		}
		 ...
		 */
		protected function redirectUserAuthorization($token,$callbackUrl, $cacheType, $custoHandlers){
			try{
				switch($cacheType){
					case self::TOKEN_CACHE_SESSION:
						...
						break;
					case self::TOKEN_CACHE_CUSTOM:
						call_user_func($custoHandlers['saveRequestToken'],$token);
				}
				$o = new OAuth($this->settings->key, $this->settings->secret, OAUTH_SIG_METHOD_HMACSHA1);
		 ...
		 * @return object token
		 */
		protected function obtainUserAuthorationAccessToken($cacheType, $custoHandlers) {
			switch($cacheType){
				case self::TOKEN_CACHE_SESSION:
					$requestToken = $_SESSION['PcoRequestToken'];
					break;
				case self::TOKEN_CACHE_CUSTOM:
					$requestToken = call_user_func($custoHandlers['getRequestToken']);
			}
		...
		/**
		 * attempt to connect to oauth based authentication using cached tokens (no new login)
		 * @param const $cacheType
		 * @param array $custoHandlers
		 * @return boolean
		 */
		public function connect($cacheType = self::TOKEN_CACHE_SESSION, $custoHandlers = NULL) {
		...
		public function login($callbackUrl, $cacheType = self::TOKEN_CACHE_SESSION, $custoHandlers = NULL) {
			...
			// fetch cached token (if any)
			$cachedLogin = $this->connect($cacheType, $custoHandlers);
			if ($cachedLogin) return true;
			
			// else handle callback (if any)
			if (isset($_GET['oauth_token'])) {
				$token = $this->obtainUserAuthorationAccessToken($cacheType, $custoHandlers);
				...
				$this->redirectUserAuthorization($token, $callbackUrl, $cacheType, $custoHandlers);
			}
